...
function buildSlackPayload(result: WorkflowResult): object {
	...
	const sorted = [...result.findings].sort(
		(a, b) =>
			(SEVERITY_SCORES[b.severity] ?? 0) - (SEVERITY_SCORES[a.severity] ?? 0),
	);
...
export async function sendWebhookNotification(
	...
	if (config.minSeverity) {
		const minIdx = ["INFO", "LOW", "MEDIUM", "HIGH", "CRITICAL"].indexOf(
			config.minSeverity,
		);
		const filtered: Finding[] = result.findings.filter((f) => {
			const idx = ["INFO", "LOW", "MEDIUM", "HIGH", "CRITICAL"].indexOf(
				f.severity,
			);
			return idx >= minIdx;
		});
	...
	switch (format) {
		case "slack":
			payload = buildSlackPayload(filteredResult);
			break;
		case "discord":
			payload = buildDiscordPayload(filteredResult);
			break;
		case "teams":
			payload = buildTeamsPayload(filteredResult);
			break;
		default:
			payload = buildJsonPayload(filteredResult);
			break;
	}
	...
	if (config.secret) {
		headers["X-Vulnhuntr-Signature"] =
			`sha256=${signPayload(body, config.secret)}`;
	}
