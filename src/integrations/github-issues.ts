...
function issueBody(finding: Finding): string {
	const lines: string[] = [
		...
		...(finding.start_line > 0
			? [
					`| **Lines** | ${finding.start_line}–${finding.end_line || finding.start_line} |`,
				]
			: []),
		"",
		"### Analysis",
...
async function findDuplicate(
	...
	const query = encodeURIComponent(
		`repo:${config.owner}/${config.repo} "${title}" in:title`,
	);
	...
	if (!res.ok) {
		console.warn(
			`GitHub search API returned ${res.status}: ${await res.text()}`,
		);
		return null;
	}
...
async function createIssue(
	...
	if (config.labels?.length)
		payload.labels = [...config.labels, finding.severity.toLowerCase()];
...
export async function createIssuesForFindings(
	...
	for (const finding of findings) {
		...
		if (sevIdx < minIdx) {
			result.skipped.push({
				finding,
				reason: `Below minimum severity ${config.minSeverity}`,
			});
			continue;
		}
