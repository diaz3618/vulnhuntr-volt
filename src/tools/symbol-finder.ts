...
function buildSearchPatterns(parts: string[]): RegExp[] {
  ...
  if (parts.length === 1) {
    ...
    patterns.push(new RegExp(`^\\s*(?:async\\s+)?def\\s+${escapeRegex(name)}\\s*\\(`));
  } else if (parts.length === 2) {
    const [className, methodName] = parts;
    // First try to find the class, then the method within it
    patterns.push(new RegExp(`^\\s*class\\s+${escapeRegex(className)}\\s*[:(]`));
...
function extractDefinitionBlock(lines: string[], startIdx: number): string {
  ...
  for (let i = startIdx + 1; i < lines.length && maxLines > 0; i++) {
    ...
    if (currentIndent <= defIndent && trimmed !== "") {
      ...
      if (trimmed.startsWith("@") || trimmed.startsWith("class ") || trimmed.startsWith("def ")) {
        break;
      }
      break;
    }
...
export const resolveSymbolTool = createTool({
  ...
  parameters: z.object({
    ...
    name: z.string().describe("Symbol name (e.g., 'ClassName' or 'ClassName.method_name')"),
    code_line: z
      .string()
...
export const resolveSymbolsBatchTool = createTool({
  ...
  parameters: z.object({
    ...
    all_files: z
      .array(z.string())
      .describe("List of all Python file paths"),
  }),
  execute: async ({ repo_path, symbols, all_files }) => {
    const results = symbols.map((sym) => {
      const result = extractSymbol(sym.name, sym.code_line, all_files, repo_path);
      if (!result) {
        return {
          found: false,
          name: sym.name,
